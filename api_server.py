...
def init_db():
    ...
    ''')
    # Eski DB'ye plain_key kolonu yoksa ekle
    try:
        conn.execute("ALTER TABLE licenses ADD COLUMN plain_key TEXT")
        conn.commit()
    except Exception:
        pass
    conn.commit()
...
def generate():
    ...
    for _ in range(count):
        ...
        try:
            conn.execute(
                "INSERT INTO licenses (key_hash, key_data, plain_key, created_date, expiry_date, is_active) VALUES (?,?,?,?,?,?)",
                (khash, enc, key, now.isoformat(), expiry.isoformat(), 1)
            )
            conn.commit()
...
def list_keys():
    ...
    conn = sqlite3.connect(DB_FILE)
    rows = conn.execute(
        "SELECT id, plain_key, key_hash, created_date, expiry_date, is_active FROM licenses ORDER BY id DESC"
    ).fetchall()
    conn.close()
    keys = []
    for row in rows:
        rid, plain_key, khash, created, expiry, active = row
        try:
            expired = datetime.fromisoformat(expiry) < datetime.now()
        except Exception:
            expired = False
        status = "iptal" if not active else ("suresi_dolmus" if expired else "aktif")
        # plain_key yoksa hash'ten kısa gösterim yap
        display_key = plain_key if plain_key else (khash[:8].upper() + "-????-????-????")
        keys.append({
            "id":      rid,
            "key":     display_key,
            "created": created[:16],
